...
<header id="masthead" class="fixed top-0 left-0 w-full z-50 bg-transparent py-11 ">
	...
	<nav id="mobile-nav" class="lg:hidden ml-auto" x-data="{open: false}" >
		...
		<div x-show="open"  x-transition class="absolute w-full h-screen bg-[#480634] left-0 top-0">
			<div class="h-full grid place-content-center gap-10">
			<?php
				if ( is_front_page() ) :
					?>
					<img class="w-[240px]" src="<?php bloginfo('template_url') ?>/assets/SVG/logo-white-vertical.svg" />
					<!-- <h1><?php bloginfo( 'name' ); ?></h1> -->
					<?php
				else :
					?>
	...
	<nav id="site-navigation" class="max-lg:hidden" aria-label="<?php esc_attr_e( 'Main Navigation', 'supreme-hotel-theme' ); ?>">
